...
TEST_CASE("Test Impl Class Functions") {
  ...
  // create particle structure with 5 particles
  std::unique_ptr<pumitally::PumiTallyImpl> p_pumi_tallyimpl =
      std::make_unique<pumitally::PumiTallyImpl>(temp_file_name, num_ptcls,
                                                 argc, argv);
  ...
  { // * Check if flux is accumulated properly if particles *move again*
    ...
    { //* Check flux
      ...
      printf("The fluxes after 2nd move \nelem_id[found, expected] \n%d[%f,%f] "
             "\n%d[%f,%f] \n%d[%f,%f] \n%d[%f,%f] \n%d[%f,%f] \n%d[%f,%f]\n",
             0, flux_host[0], flux_expected[0], 1, flux_host[1],
             flux_expected[1], 2, flux_host[2], flux_expected[2], 3,
             flux_host[3], flux_expected[3], 4, flux_host[4], flux_expected[4],
             5, flux_host[5], flux_expected[5]);
