...
TEST_CASE("Test reader") {
  Kokkos::initialize();
  {
    ...
    auto energyGroupEdges_host =
        Omega_h::HostRead<Omega_h::Real>(energyGroupEdges);
    ...
    for (int i = 0; i < mgxs.getNumEnergyGroups() + 1; ++i) {
      ...
      REQUIRE_THAT(energyGroupEdges_host[i],
                   Catch::Matchers::WithinAbs(expected_edges[i], margin));
    }
