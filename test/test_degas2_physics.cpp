...
TEST_CASE("Test Degas2 Physics Functions") {
...

  {
    ...
    Kokkos::parallel_for(
        "Set energies", numParticles,
        KOKKOS_LAMBDA(int i) { physics.set_energy(particles(i), energy); });
    ...
    outfile << "Standard Deviation of Distance (cm): " << sdl
            << std::endl; // 2.38
    ...
    outfile << "Standard Deviation of Mean x Direction: " << sdux
            << std::endl; // 0.579

    outfile << std::left << std::setw(10) << "Particle #" << std::setw(16)
            << "Velocity(m/s)" << std::setw(10) << "Weight" << std::setw(10)
            << "X(cm)" << std::setw(10) << "Y(cm)" << std::setw(10) << "Z(cm)"
            << std::setw(10) << "X_Dir" << std::setw(10) << "Y_Dir"
            << std::setw(10) << "Z_Dir" << '\n';

    for (int i = 0; i < numParticles; ++i) {
      outfile << std::left << std::setw(10) << i << std::setw(16)
              << (1.0 / output(i).alpha) << std::setw(10) << output(i).weight
              << std::setw(10) << output(i).position[0] << std::setw(10)
              << output(i).position[1] << std::setw(10) << output(i).position[2]
              << std::setw(10) << output(i).direction[0] << std::setw(10)
              << output(i).direction[1] << std::setw(10)
              << output(i).direction[2] << '\n';
    }
    ...
    // with 1000 samples. But we should use something like 2-3 standard
    // deviations with more than 99% confidence.
