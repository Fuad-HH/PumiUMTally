...
bool checkOrderZero(const H5::H5File &file, const std::vector<std::string> &materialNames);
bool checkNonFissionable(const H5::H5File &file, const std::vector<std::string> &materialNames);
void readOtherAttributes(H5::H5File &file, const std::vector<std::string> &materialNames);
...
std::vector<std::string> readTemperatureForAllMaterials(const H5::H5File &file, const std::vector<std::string> &materialNames);
...
void MultiGroupXS::fillXS(const H5::H5File &file) {
    ...
    for (int matId = 0; matId < materialNames_.size(); ++matId) {
        ...
        for (int tempId = 0; tempId < temperatureNames_.size(); ++tempId) {
            ...
            const auto scatteringPath = temperaturePath + "/scatter_data/scatter_matrix";
...
std::vector<std::string> readTemperatureForAllMaterials(const H5::H5File &file, const std::vector<std::string> &materialNames) {
    ...
    for (int i = 1; i < temperatures.size(); i++) {
        if (temperatures[i] != temperatures[0]) {
            std::string errorMsg = "Materials have different temperatures: " + temperatures[i][0] + " and " + temperatures[0][0];
...
void MultiGroupXS::print() {
    ...
    auto energyGroupEdges_h = Omega_h::HostWrite<Omega_h::Real>(energyGroupEdges_);
